...
| **Verification Required** | **Before the first Pet Definition Data `/dev-story`** (local 4.7.1): (1) `ResourceLoader` runs `@export` property setters while loading a `.tres`, and `_locked` is still `false` at that point (source-confirmed by the godot-specialist 2026-09-23: the loader calls `Object::set()` per property after `_init()`, and a typed array arrives as one whole-array `set()`; one test pins it); (2) `Object.set(&"field", v)` on a locked definition goes through the GDScript setter and is rejected; (3) a `Dictionary` keyed by `StringName` resolves when a lookup's `StringName` parameter is passed a `String` literal (the typed parameter converts it); (4) `Array[T].make_read_only()` rejects `append`, `[i] =` and `sort()` with a logged error and no crash. *Result 2026-09-24: #1–#3 pass; #4 partial — the array is never changed and the process never crashes, but `[i] =` raises a GDScript runtime error that aborts the calling function (see §2).* **Before the first export build:** (5) a `.tres` reached only through the manifest's `ext_resource` references is included in an Android and an iOS export with the default "export all resources" filter **and** with a "selected scenes/resources" filter. **On a real exported build, not editor play:** (6) `catalog.state == READY` and `get_all_species().size() > 0` on Android and iOS. That the file is in the PCK doesn't prove the typed array deserialised non-empty (godot/godot#98798). |
...
- The code enums (PDD Core Rule 1) are `Need.Id`, `CareAction.Id` and `LifeStage.Id`, in `src/core/pet_data/pet_enums.gd` (`Need`), `care_action.gd` and `life_stage.gd` — one file per enum, because GDScript allows one global `class_name` per script and callers use the bare `Need.Id` form (amended 2026-09-24, PDD Story 001). Needs and actions are **named fields** on the profiles, not enum-indexed arrays, so the inspector labels them and a mis-ordered array can't silently swap two needs.
...
- `DefinitionResource.lock() -> void` sets `_locked = true` and then walks `get_property_list()` for `PROPERTY_USAGE_STORAGE` properties. A `DefinitionResource` value gets `lock()`. An `Array` value gets `make_read_only()` and then `lock()` on each `DefinitionResource` element. Locking is idempotent: `lock()` returns at once if `_locked` is already set, which also makes a cyclic graph safe (amended 2026-09-24).
- **Locked arrays fail two ways** (Verification #4, 4.7.1): `append()`, `sort()` and the other mutators are C++ rejections that log an error and let the caller carry on, but an indexed write `arr[i] = v` is a GDScript runtime error that **aborts the calling function**. The array is unchanged either way and the app doesn't crash, but the caller's remaining statements don't run, which falls short of Pillar 2's intent. Closed by CI lint (below) rather than a runtime guard, since array writes can't be setter-guarded.
...
- **Not covered by the guard, closed by CI lint instead:**
  ...
  - Indexed writes into a definition's array field (`<definition>.<array_field>[i] = …`) abort the caller (see above). CI fails on an indexed assignment whose target is a field of a `*Definition`, `NeedProfile`, `NeedParams`, `CareProfile` or `FormRule*` value under `src/` (pattern `definition_array_index_write`, added 2026-09-24).
