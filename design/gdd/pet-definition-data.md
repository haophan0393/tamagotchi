...
3. **Definitions are immutable at runtime — enforced, not assumed.** The full catalog is loaded once at boot, validated, and thereafter shared read-only by reference. No consumer mutates a definition or holds a private copy of a value. Because a plain GDScript `@export` Resource cannot enforce this on its own, immutability is a contract backed by the mechanism fixed in ADR-0002 §2 (`docs/architecture/adr-0002-pet-catalog-loading-and-immutability.md`, 2026-09-23): **setter guards** on every exported definition field, which `push_error` and ignore the write once the catalog has recursively `lock()`ed the tree at `Ready`, plus read-only arrays, plus CI lint for the gaps a setter can't cover (`SpriteFrames` mutators, `duplicate()`/`duplicate_deep()`, and `load()` of pet data outside the loader). A test must exist that demonstrates a post-`Ready` write attempt is rejected or detected (see Acceptance Criteria). A pet's *current* need levels, stage, days-in-stage, care history and chosen form are runtime state owned by Save & Persistence.
...
- **GIVEN** a catalog in `Ready` state (ADR-0002 §2 setter guards), **WHEN** test code attempts to write any definition field after `Ready` — by assignment, `set()`, or `append` on an array field — **THEN** the write is ignored, the value is unchanged and an error is logged, and a test demonstrating this exists before the first dependent system is implemented (Core Rule 3).
...
- **GIVEN** a loaded catalog, **WHEN** calling `get_species(id)` / `get_form(id)` with an unknown id, **THEN** each returns `null`, never a silent default. A `String` argument (`"nope"`) behaves the same as a `StringName` (Core Rule 13; ADR-0002 §4).
...
| 9 | Unknown-id lookup result is not pinned (Rule 13 says "null or explicit error"). Fix: pin `null` in Rule 13 and the matching AC | qa-lead | When the catalog API is implemented (OQ#2 ADR) — **review blocker accepted 2026-09-22** — **RESOLVED 2026-09-23**: `null`, pinned in Rule 13 and the AC |
