...
- **GIVEN** a `TimeService` wired to `SystemTimeSource` (production), **WHEN** `get_now_utc()` is called twice one second apart, **THEN** the second call returns a value ≥ the first by at least 1.
...
- **GIVEN** a `TimeService` instance, **WHEN** any of its public methods are called, **THEN** no call reaches `Time.*` or `OS.*` directly except through the injected `TimeSource` — verified by using a `FakeTimeSource` in every test and confirming results never vary with real wall-clock time during the test run.
